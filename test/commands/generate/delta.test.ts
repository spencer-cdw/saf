...
describe('Test generate delta', () => {
  ...
  fse.copydir(path.resolve('./test/sample_data/delta-test/profile'), tmpobj.name);
  
  test
  .command(['generate delta', '-i', path.resolve(tmpobj.name) + ' ' + path.resolve(`${tmpobj.name}/profile.json`), '-r', path.resolve(`${tmpobj.name}/report.md`)])
